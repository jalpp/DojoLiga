...
public class Main extends ListenerAdapter {
    ...
    private final DiscordAction DiscordReactor = new DiscordAction();

    public static final String botToken = dotenv.get("LICHESS_BOT_TOKEN");

    private static MongoCollection<Document> collection = null;

    private static final Client client = Client.basic();

    private static  MongoCollection<Document> chesscomplayers = null;

    private static MongoCollection<Document> computedId = null;

    private static MongoCollection<Document> u1800Playerswiss = null;

    private static MongoCollection<Document> swissLeagueCollection = null;

    private static MongoCollection<Document> arenaLeagueCollection = null;

    private static MongoCollection<Document> RRplayercollection = null;

    private static MongoCollection<Document> RRcollection = null;

    private static MongoCollection<Document> u1800swissCollection = null;


    public static void main(String[] args) {


        String beta = dotenv.get("DOJO_LIGA_BETA");
        String prod = dotenv.get("DOJO_LIGA_PROD");
        ...
        try {
            ...
            collection = MongoConnect.getLichessplayers();
            chesscomplayers = MongoConnect.getChesscomplayers();
            computedId = MongoConnect.getComputedId();
            swissLeagueCollection = MongoConnect.getSwissLeagueCollection();
            arenaLeagueCollection = MongoConnect.getArenaLeagueCollection();
            u1800Playerswiss = MongoConnect.getU1800Playerswiss();
            ...
            commands.addCommands(Commands.slash("pairings", "View pairings for the time command is run!")
                    .addOption(OptionType.STRING, "request-url", "Input Lichess arena/swiss URL.", true)); // fix
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        switch (event.getName()){
            ...
            case "stream" -> event.reply("https://www.twitch.tv/chessdojolive").queue();
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        switch (event.getMessage().getContentRaw()){
            ...
            case "/ticket" -> new TicketManager().createEntryTicketFromMsg(event);
