...
public class RoundRobinCrosstable {
    ...
    public void updateCrossTableScores(String player1, String player2, GameState state) {
        ...
        if (!(players.size() < 3)) {
            ...
            switch (state) {
                case DRAW -> {
                    actualList.get(player1Index).add(player2Index, "1/2");
                    actualList.get(player2Index).add(player1Index, "1/2");
                }
                case PLAYER_ONE_WON, PLAYER_TWO_LOST -> {
                    actualList.get(player1Index).add(player2Index, "1");
                    actualList.get(player2Index).add(player1Index, "0");
                }

                case PLAYER_ONE_LOST, PLAYER_TWO_WON -> {
                    actualList.get(player1Index).add(player2Index, "0");
                    actualList.get(player2Index).add(player1Index, "1");
                }
            }
